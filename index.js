...
async function performSwap(fromToken, toToken, amount) {
    log(chalk.yellow(`Mempersiapkan swap ${amount} ${fromToken.name} ke ${toToken.name} via Uniswap V2 Router...`));
    ...
    try {
        ...
        const toTokenDecimals = await new ethers.Contract(toToken.address, config.ERC20_ABI, provider).decimals();
        log(`Estimasi output: ${ethers.formatUnits(amountsOut[1], toTokenDecimals)} ${toToken.name}`);
        ...
        if (error.data) {
            log(chalk.red(`Error data: ${error.data}`));
        }
    }
}
...
async function performAddLiquidity(tokenA, tokenB, amountA) {
    ...
    try {
        const amountsOut = await router.getAmountsOut(amountADesired, [tokenA.address, tokenB.address]);
